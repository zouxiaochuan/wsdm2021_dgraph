...
class DygDataset(torch.utils.data.Dataset):
    ...
    def get_pair_feat_extra(self, current_edge_type, pair_edges):
        ...
        if len(pair_edges) > 0:
            ...
            if 'edge_type_feat_file' in self.config:
                pair_edge_types_feat = self.edge_type_feat[pair_edge_types] + 1
            else:
            ...
            if 'edge_type_feat_file' in self.config:
                pair_feat = np.zeros((0, 4), dtype='int64')
            else:
    ...
    def get_node_history_feat_extra(self, history_edges, current_edge_type, other_node):
        ...
        if 'node_feat_file' in self.config:
            feat = np.zeros((len(history_edges), 13), dtype='int64')

            for i, (eid, direction) in enumerate(history_edges):
                history_edges_type = self.edge_types[eid]
                feat[i, 0:3] = self.edge_type_feat[history_edges_type] + 1
                if direction == 0:
                    node_type = self.dst_nodes[eid]
                    feat[i, 3:11] = self.node_feat[node_type] + 1
                    feat[i, 11] = int(self.dst_nodes[eid] == other_node)
                else:
                    node_type = self.src_nodes[eid]
                    feat[i, 3:11] = self.node_feat[node_type] + 1
                    feat[i, 11] = int (self.src_nodes[eid] == other_node)

                feat[i, 12] = int(self.edge_types[eid] == current_edge_type)
